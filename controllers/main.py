...
class BiblicalGameController(http.Controller):
    ...
    def user_guide(self, **kwargs):
        ...
        return request.render(
            "biblical_game.biblical_game_guide_template",
            {"base_url": request.httprequest.url_root},
        )

    # ===== ROUTES DU GUIDE UTILISATEUR =====
    @http.route("/web/biblical-game-guide", type="http", auth="user", website=True)
    def biblical_game_guide_home(self, **kwargs):
    ...
    @http.route(
        "/web/biblical-game-guide/installation", type="http", auth="user", website=True
    )
    def biblical_game_guide_installation(self, **kwargs):
        """Page d'installation du guide utilisateur"""
        return request.render(
            "biblical_game.biblical_game_installation_template",
            {"base_url": request.httprequest.url_root},
        )

    @http.route(
        "/web/biblical-game-guide/bible-management",
        type="http",
        auth="user",
        website=True,
    )
    def biblical_game_guide_bible_management(self, **kwargs):
        """Page de gestion de la Bible du guide utilisateur"""
        return request.render(
            "biblical_game.biblical_game_bible_management_template",
            {"base_url": request.httprequest.url_root},
        )

    @http.route(
        "/web/biblical-game-guide/game-stages", type="http", auth="user", website=True
    )
    def biblical_game_guide_game_stages(self, **kwargs):
        """Page des étapes du jeu du guide utilisateur"""
        return request.render(
            "biblical_game.biblical_game_stages_template",
            {"base_url": request.httprequest.url_root},
        )

    @http.route(
        "/web/biblical-game-guide/configuration", type="http", auth="user", website=True
    )
    def biblical_game_guide_configuration(self, **kwargs):
        """Page de configuration du guide utilisateur"""
        return request.render(
            "biblical_game.biblical_game_configuration_template",
            {"base_url": request.httprequest.url_root},
        )
    ...
    def start_biblical_game(self):
        ...
        try:
            ...
            settings_count = (
                request.env["biblical.game.settings"]
                .sudo()
            ...
            _logger.info(
                f"Chargement de la page principale - Paramètres: {settings_count}, Versets: {verses_count}"
            )
        ...
        try:
            ...
            for setting in settings:
                try:
                    ...
                    _logger.debug(
                        f"Paramètre ajouté: {setting.name} - {len(book_info)} livres"
                    )

                except Exception as e:
                    _logger.error(
                        f"Erreur lors du traitement du paramètre {setting.name}: {e}"
                    )
                    continue
            ...
            _logger.info(
                f"Paramètres récupérés avec succès: {len(settings_data)} éléments"
            )
            return {"success": True, "data": settings_data, "count": len(settings_data)}

        except Exception as e:
            _logger.error(
                f"Erreur lors de la récupération des paramètres: {e}", exc_info=True
            )
            return {
                "success": False,
    ...
    def get_setting_details(self, setting_id):
        ...
        try:
            ...
            if setting.book_ids:
                available_verses = (
                    request.env["biblical.game.verse"]
                    .sudo()
                    .search_count([("chapter_id.book_id", "in", setting.book_ids.ids)])
                )
            else:
                available_verses = 0
    ...
    def get_stage_thresholds(self, level):
        ...
        try:
            level = int(level)
            stage = (
                request.env["biblical.game.stage"]
                .sudo()
                .search([("level", "=", level), ("is_active", "=", True)], limit=1)
            )
    ...
    def create_game_session(self, setting_id):
        ...
        try:
            ...
            if request.env.user._is_public():
                ...
                existing_session = (
                    request.env["biblical.game.session"]
                    .sudo()
                    .search(
                ...
                try:
                    ...
                    session = (
                        request.env["biblical.game.session"].sudo().create(session_vals)
                    )
                    session_data = {
                        "session_id": session.id,
                        "user_name": session.user_id.name,
    ...
    def end_game_session(
        self, session_id, final_score, questions_answered=0, correct_answers=0
    ):
        ...
        try:
            ...
            if (
                ...
                and session.user_id.id != request.env.user.id
            ):
                return {
                    "success": False,
    ...
    def _calculate_medal(self, thresholds, score):
        ...
        silver_threshold = thresholds.get(
            "silver_threshold", thresholds.get("silver", 50)
        )
        bronze_threshold = thresholds.get(
            "bronze_threshold", thresholds.get("bronze", 20)
        )
        ...
        if score >= gold_threshold:
            ...
            progress = (
                (score - silver_threshold) / (gold_threshold - silver_threshold) * 100
            )
            ...
            progress = (
                (score - bronze_threshold) / (silver_threshold - bronze_threshold) * 100
            )
        else:
            medal = "none"
            emoji = "❌"
    ...
    def debug_test_settings(self):
        ...
        try:
            ...
            settings_count = (
                request.env["biblical.game.settings"].sudo().search_count([])
            )
            ...
            sample_settings = (
                request.env["biblical.game.settings"].sudo().search([], limit=3)
            )
            sample_data = []
            ...
            for setting in sample_settings:
                try:
                    ...
                    sample_data.append({"id": setting.id, "error": str(e)})
